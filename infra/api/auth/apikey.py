...
def require_authentication(request: Request) -> str:
    ...
    logger.info("Successful API key authentication", extra={
        "path": request.url.path,
        "method": request.method,
        "client": request.client.host if request.client else "unknown"
    })
...
def optional_authentication(request: Request) -> Optional[str]:
    ...
    if not validate_api_key(api_key):
        logger.warning("Invalid API key attempted (optional auth)", extra={
            "path": request.url.path,
            "method": request.method,
            "client": request.client.host if request.client else "unknown"
        })
    ...
    logger.info("Successful optional API key authentication", extra={
        "path": request.url.path,
        "method": request.method,
        "client": request.client.host if request.client else "unknown"
    })
