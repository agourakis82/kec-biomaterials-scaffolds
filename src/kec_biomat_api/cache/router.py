...
async def cache_get(key: str, cache: CacheManager = Depends(get_cache)):
    ...
    try:
        ...
        if result is not None:
            ...
            return CacheGetResponse(value=None, hit=False, backend=None, metadata=None)
...
async def cache_set(request: CacheSetRequest, cache: CacheManager = Depends(get_cache)):
    ...
    try:
        ...
        success = await cache.set(
            request.key, request.value, ttl=request.ttl, metadata=metadata
        )
...
async def cache_delete(key: str, cache: CacheManager = Depends(get_cache)):
...
async def cache_delete_multiple(
    request: CacheDeleteRequest, cache: CacheManager = Depends(get_cache)
):
...
async def cache_exists(key: str, cache: CacheManager = Depends(get_cache)):
...
async def cache_keys(
    ...
    try:
        ...
        return {"keys": keys, "count": len(keys), "pattern": pattern, "limit": limit}
...
async def cache_backend_comparison(metrics: CacheMetrics = Depends(get_metrics)):
...
async def list_strategies(strategies: InvalidationManager = Depends(get_strategies)):
...
async def activate_strategy(
    strategy_name: str, strategies: InvalidationManager = Depends(get_strategies)
):
...
async def cache_health(
    ...
    try:
        ...
        # Status geral
        all_healthy = all(
            backend.get("status") == "healthy" for backend in backend_health.values()
        )
        ...
        return CacheHealthResponse(
            status="unhealthy", backends={}, metrics={}, strategy=None
        )
