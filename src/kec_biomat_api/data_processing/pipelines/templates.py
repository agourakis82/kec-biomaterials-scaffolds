...
class SimpleETLTemplate(PipelineTemplate):
    ...
    def create_config(self, **kwargs) -> PipelineConfig:
        ...
        source = create_file_source(
            "input", input_file, format=input_format, chunk_size=chunk_size
        )
        ...
        quality = QualityConfig(
            enable_validation=enable_validation, enable_profiling=True
        )
...
class DataValidationTemplate(PipelineTemplate):
    ...
    def create_config(self, **kwargs) -> PipelineConfig:
        ...
        # Fonte de dados
        source = create_file_source(
            "input", input_file, format=kwargs.get("input_format", DataFormat.JSON)
        )
...
class DataCleaningTemplate(PipelineTemplate):
    ...
    def create_config(self, **kwargs) -> PipelineConfig:
        ...
        sink = create_file_sink(
            "output", output_file, format=kwargs.get("output_format", DataFormat.JSON)
        )
        ...
        quality = QualityConfig(enable_validation=True, enable_profiling=True)
        ...
        return PipelineConfig(
            ...
            processing=processing,
            quality=quality,
            metadata={**kwargs, "cleaning_rules": cleaning_rules},
        )
...
class FormatConversionTemplate(PipelineTemplate):
    ...
    def create_config(self, **kwargs) -> PipelineConfig:
        ...
        if not all([input_file, output_file, input_format, output_format]):
            raise ValueError(
                "input_file, output_file, input_format, and output_format are required"
            )

        # Fonte e destino
        source = create_file_source("input", input_file, format=input_format)

        sink = create_file_sink("output", output_file, format=output_format)
...
class MultiFileETLTemplate(PipelineTemplate):
    ...
    def create_config(self, **kwargs) -> PipelineConfig:
        ...
        return PipelineConfig(
            ...
            sinks=[sink],
            processing=processing,
            metadata={**kwargs, "input_files": input_files},
        )
...
def clean_data_file(
    input_file: str, output_file: str, cleaning_rules: List[Dict], **kwargs
):
