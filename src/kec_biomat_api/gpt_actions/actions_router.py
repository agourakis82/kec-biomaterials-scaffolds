...
class KECAnalysisRequest(BaseModel):
    ...
    graph_data: Optional[Dict[str, Any]] = Field(
        None, description="Graph data em formato NetworkX JSON"
    )
    config_overrides: Optional[Dict[str, Any]] = Field(
        None, description="Override de configuração KEC"
    )
...
class MemoryQueryRequest(BaseModel):
    ...
    memory_type: str = Field(
        "conversation", description="Tipo: conversation, project, discovery"
    )
    ...
    try:
        ...
        if request.use_iterative:
            ...
            iterative_search = IterativeSearch(
                IterativeConfig(max_iterations=3), rag_engine=rag_engine
            )
...
async def tree_search_gpt(request: TreeSearchRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        # Configura PUCT
        config = PUCTConfig(
            default_budget=request.budget, max_depth=request.max_depth, c_puct=1.414
        )
        ...
        # Executa busca
        result_node = await puct_search.search(
            request.initial_state, budget=request.budget
        )
...
async def memory_query_gpt(request: MemoryQueryRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if request.memory_type == "conversation":
            ...
            conversations = (
                await memory_system.conversation_memory.retrieve_relevant_context(
                    query=request.query,
                    max_results=10,
            ...
            findings = await memory_system.scientific_discovery.get_recent_discoveries(
                hours=request.time_window_days * 24, min_relevance=0.5
            )
        ...
        return {"success": False, "error": str(e), "query": request.query}
...
async def get_project_status_gpt() -> Dict[str, Any]:
    ...
    try:
        ...
        conversation_summary = (
            await memory_system.conversation_memory.get_conversation_summary(days=1)
        )
        discovery_summary = (
            await memory_system.scientific_discovery.generate_discovery_report(hours=24)
        )
        ...
        return {
            ...
            "current_status": {
                ...
                "systems_active": len(
                    [
                        s
                        for s in system_health["subsystems"].values()
            ...
            "recent_activity": {
                ...
                "files_modified": len(
                    project_context["immediate_context"]["files_needing_attention"]
                ),
                "active_tasks": len(
                    project_context["immediate_context"]["active_tasks"]
                ),
            },
...
async def get_system_health_gpt() -> Dict[str, Any]:
    ...
    try:
        ...
        simplified_health = {
            ...
            "four_steps_completed": len(
                [s for s in health["four_steps"].values() if s["status"] == "completed"]
            ),
            "total_steps": len(health["four_steps"]),
            "memory_active": True,
            "discovery_active": health["subsystems"]
            .get("scientific_discovery", {})
            .get("running", False),
            "timestamp": "2025-09-20T01:01:00Z",
        }
    ...
    try:
        ...
        if "kec_metrics" in data:
            analysis = await helio_service.analyze_with_pcs_integration(
                data["kec_metrics"], metadata=data.get("metadata", {})
            )
...
def _interpret_kec_metrics(metrics: Dict[str, float], graph) -> Dict[str, str]:
    ...
    if sigma_val > 2.0:
        interpretation["small_world"] = (
            "Forte comportamento small-world - eficiente para transporte"
        )
    elif sigma_val > 1.2:
        interpretation["small_world"] = "Comportamento small-world moderado"
    else:
        interpretation["small_world"] = (
            "Comportamento small-world fraco - pode limitar difusão"
        )
    ...
    if sigma_val > 1.5 and h_val > 2.0:
        interpretation["recommendation"] = (
            "Excelente para aplicações biomédicas - alta eficiência e complexidade"
        )
    elif sigma_val < 1.2 or h_val < 1.0:
        interpretation["recommendation"] = "Considerar otimização da arquitetura porosa"
    else:
        interpretation["recommendation"] = (
            "Propriedades adequadas para aplicação target"
        )
