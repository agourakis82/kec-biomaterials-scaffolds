...
class PermissionManager:
    ...
    async def check_multiple_permissions(
        self, user: User, permissions: List[str], require_all: bool = True
    ) -> bool:
        ...
        try:
            if permission in self.system_permissions:
                if permission not in role.permissions:
                    ...
                    logger.info(
                        f"Permission '{permission}' granted to role '{role.name}'"
                    )
                    return True
            else:
    ...
    async def revoke_permission_from_role(self, role: Role, permission: str) -> bool:
        ...
        try:
            if permission in role.permissions:
                ...
                self._invalidate_cache()
                logger.info(
                    f"Permission '{permission}' revoked from role '{role.name}'"
                )
                return True
            return False
    ...
    def _check_hierarchical_permission(
        self, user_permissions: Set[str], required_permission: str
    ) -> bool:
    ...
    def list_permissions_by_resource(self, resource: str) -> List[str]:
        ...
        return [
            perm
            for perm in self.system_permissions.keys()
